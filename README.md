...
My code utilizes an escape time algorithm which determines how quickly a given number grows (if it grows at all). This is done by looking at which iteration the series exceeds a certain threshold. If color is applied to each point based on their iteration value, the mandelbrot appears.\
